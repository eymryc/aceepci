...
import { eventsApi, newsApi, type NewsArticle, NEWS_CATEGORIES, publicOptionsApi } from "@/lib/api";
  ...
  useEffect(() => {
    ...
    newsApi
      ...
      .then((data) => {
        ...
        setTitle(data.title ?? "");
        setSlug(data.slug ?? "");
        setCategory((data.category as string | null) ?? "");
        ...
        setLinkedEventId(data.linked_event_id != null ? String(data.linked_event_id) : "");
        setCustomCtaLabel(data.custom_cta_label ?? "");
        setCustomCtaUrl(data.custom_cta_url ?? "");
            ...
            <AdminCard padding="none">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="block text-sm font-medium text-foreground mb-1">Avatar auteur</label>
                  {article?.author_avatar_url && (
                    <p className="text-xs text-muted-foreground">
                      Avatar actuel : <span className="underline break-all">{article.author_avatar_url}</span>
                    </p>
                  )}
                  <input
                    type="file"
          ...
          <div className="space-y-6 min-w-0">
            <AdminCard padding="none">
              ...
              <div className="p-6 space-y-3">
                {article?.image_url && (
                  <p className="text-xs text-muted-foreground">
                    Image actuelle : <span className="underline break-all">{article.image_url}</span>
                  </p>
                )}
                <input
                  type="file"
            ...
            <AdminCard padding="none">
              ...
              <div className="p-6 space-y-3">
                {article?.gallery_images && article.gallery_images.length > 0 && (
                  <p className="text-xs text-muted-foreground">
                    Galerie actuelle : {article.gallery_images.length} image(s)
                  </p>
                )}
                <input
                  type="file"
