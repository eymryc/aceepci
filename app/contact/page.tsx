...
import { submitContactMessage, fetchContactInfoPublished, formatApiErrorMessage, type ContactMessagePayload, type ContactInfo } from "@/lib/api";

const SUBJECT_OPTIONS = [
  { value: "", label: "Sélectionnez un sujet" },
  { value: "adhesion", label: "Adhésion" },
  { value: "activites", label: "Activités et événements" },
  { value: "don", label: "Don et partenariat" },
  { value: "info", label: "Demande d'information" },
  { value: "autre", label: "Autre" },
];

const DEFAULT_PHONE = "(+225) 27 22 44 43 78";
const DEFAULT_EMAIL = "contact@aceepci.org";
const DEFAULT_ADDRESS = "MAPE, Boulevard de l'Université\nAbidjan-Cocody";
const DEFAULT_MAP_URL = "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3972.3527364891243!2d-3.9858447!3d5.3599517!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xfc1eb7c3c7d3c53%3A0x6a9c9c9c9c9c9c9c!2sBoulevard%20de%20l'Universit%C3%A9%2C%20Abidjan%2C%20C%C3%B4te%20d'Ivoire!5e0!3m2!1sfr!2sfr!4v1234567890123!5m2!1sfr!2sfr";
...
export default function Page() {
  ...
  useEffect(() => {
    fetchContactInfoPublished().then(setContactInfo);
  }, []);
  ...
  return (
    <div>
      ...
      {(() => {
        const phone = contactInfo?.phone?.trim() || DEFAULT_PHONE;
        const email = contactInfo?.email?.trim() || DEFAULT_EMAIL;
        const address = contactInfo?.address?.trim() || DEFAULT_ADDRESS;
        const phoneHref = phone.replace(/\s/g, "").replace(/[()]/g, "");
        return (
            ...
            <div className="relative z-10 max-w-6xl mx-auto -mt-20 sm:-mt-24">
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                ...
                <div className="group flex flex-col items-center text-center bg-white border border-border rounded-2xl p-8 sm:p-10 shadow-[0_4px_24px_rgba(24,64,112,0.06)] hover:shadow-[0_12px_40px_rgba(24,64,112,0.12)] hover:border-brand-primary/25 transition-all duration-300 sm:col-span-2 lg:col-span-1">
                  ...
                  <p className="text-brand-primary font-semibold text-sm sm:text-base leading-relaxed whitespace-pre-line">
                    {address}
                  </p>
                </div>
              </div>
      ...
      <AnimateSection className="relative py-20 sm:py-28 bg-brand-subtle overflow-hidden px-4 sm:px-6 lg:px-8">
        <div className="relative z-10 max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            ...
            <div className="bg-white border border-border rounded-2xl p-8 sm:p-10 shadow-[0_4px_24px_rgba(24,64,112,0.06)]">
              ...
              <form className="space-y-5 sm:space-y-6" onSubmit={handleSubmit}>
                ...
                <button
                  type="submit"
                  disabled={sending}
                  className="w-full py-3.5 sm:py-4 bg-gradient-to-r from-brand-primary to-brand-accent text-white rounded-lg font-semibold flex items-center justify-center gap-2 hover:opacity-95 transition-opacity shadow-[0_4px_14px_rgba(24,64,112,0.25)] disabled:opacity-70 disabled:pointer-events-none"
                >
            ...
            {/* Carte + Horaires */}
            {(() => {
              const mapUrl = contactInfo?.map_embed_url?.trim() || DEFAULT_MAP_URL;
              const addressCaption = contactInfo?.address?.trim() || DEFAULT_ADDRESS.replace(/\n/g, ", ");
              const hoursMonFri = contactInfo?.hours_mon_fri?.trim() || "8h00 - 17h00";
              const hoursSat = contactInfo?.hours_saturday?.trim() || "9h00 - 13h00";
              const hoursSun = contactInfo?.hours_sunday?.trim() || "Fermé";
              return (
                  ...
                  <div className="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-[0_4px_24px_rgba(24,64,112,0.06)]">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center py-3 border-b border-border">
                        ...
                        <span className="font-semibold text-foreground">{hoursMonFri}</span>
                      </div>
                      <div className="flex justify-between items-center py-3 border-b border-border">
                        <span className="text-muted-foreground">Samedi</span>
                        <span className="font-semibold text-foreground">{hoursSat}</span>
                      </div>
                      <div className="flex justify-between items-center py-3">
                        <span className="text-muted-foreground">Dimanche</span>
                        <span className="font-semibold text-foreground">{hoursSun}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {(() => {
        ...
        const regionalList = (contactInfo?.regional_contacts && contactInfo.regional_contacts.length > 0)
          ? contactInfo.regional_contacts.filter((r) => (r.region ?? "").trim())
          : defaultRegional;
        return (
          <AnimateSection className="relative py-20 sm:py-28 bg-white overflow-hidden px-4 sm:px-6 lg:px-8">
            <div className="relative z-10 max-w-7xl mx-auto">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground">
                    {dept.contact && (
                      <a href={`tel:${String(dept.contact).replace(/\s/g, "")}`} className="flex items-center gap-2 hover:text-brand-primary transition-colors">
      ...
      <AnimateSection className="relative py-20 sm:py-28 bg-white overflow-hidden px-4 sm:px-6 lg:px-8">
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <div className="flex justify-center gap-4 sm:gap-6 mb-14">
            {(contactInfo?.facebook_url?.trim() ? (
              <a
                href={contactInfo.facebook_url.trim()}
                target="_blank"
              ...
              <span className="w-14 h-14 sm:w-16 sm:h-16 bg-brand-primary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center text-brand-primary" aria-hidden><Facebook className="w-6 h-6 sm:w-7 sm:h-7" /></span>
            ))}
            {(contactInfo?.instagram_url?.trim() ? (
              <a
                href={contactInfo.instagram_url.trim()}
                target="_blank"
                rel="noopener noreferrer"
                className="w-14 h-14 sm:w-16 sm:h-16 bg-brand-primary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center text-brand-primary hover:bg-brand-primary hover:text-white hover:border-brand-primary transition-all duration-300"
              ...
              <span className="w-14 h-14 sm:w-16 sm:h-16 bg-brand-primary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center text-brand-primary" aria-hidden><Instagram className="w-6 h-6 sm:w-7 sm:h-7" /></span>
            ))}
            {(contactInfo?.youtube_url?.trim() ? (
              <a
                href={contactInfo.youtube_url.trim()}
                target="_blank"
                rel="noopener noreferrer"
                className="w-14 h-14 sm:w-16 sm:h-16 bg-brand-primary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center text-brand-primary hover:bg-brand-primary hover:text-white hover:border-brand-primary transition-all duration-300"
                aria-label="YouTube"
              ...
              <span className="w-14 h-14 sm:w-16 sm:h-16 bg-brand-primary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center text-brand-primary" aria-hidden><Youtube className="w-6 h-6 sm:w-7 sm:h-7" /></span>
            ))}
          </div>
          <div className="bg-brand-subtle border border-border rounded-2xl p-8 sm:p-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                href={`tel:${(contactInfo?.phone?.trim() || DEFAULT_PHONE).replace(/\s/g, "").replace(/[()]/g, "")}`}
